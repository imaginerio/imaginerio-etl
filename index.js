    ...
    photoOverlays.forEach(photo => {
        const feature = {
            ...
            properties: {
                ...
                angle: 50,
                height: parseFloat(
                    photo.Camera[0].altitude[0]
                ),
                heading: parseFloat(
                    photo.Camera[0].heading[0]
                ),
                bearing: parseFloat(
                    photo.Camera[0].heading[0]
                ),
            ...
            geometry: {
                ...
                coordinates: [
                    parseFloat(
                        photo.Camera[0].longitude[0]
                    ),
                    parseFloat(
                        photo.Camera[0].latitude[0]
                    )
                ]
            }
        };
        ...
        const bearing1 = feature.properties.bearing + parseFloat(photo.ViewVolume[0].leftFov[0])
        const bearing2 = feature.properties.bearing + parseFloat(photo.ViewVolume[0].rightFov[0])
...
const kmlToCSVWikidata = (photoOverlays) => {
    ...
    fs.writeFileSync(
        `${distDir}/${distfileName}-wikidata.csv`,
        file
    );
}
