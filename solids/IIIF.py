...
def write_manifest(info, import_omeka):
    index = info["id"].split("iiif-img/")[-1]
    print("id: ", id)
    tmp_path = "data/output/tmp"
    item = import_omeka.index[import_omeka["dcterms:identifier"] == index].tolist()[0]
    metadata_df = import_omeka.iloc[item]
    print(metadata_df["dcterms:title"])
    ...
    manifest.set_id(extendbase_url="{0}/manifest".format(str(index).replace(" ", "_")))
    ...
    if not os.path.exists(tmp_path):
        os.mkdir(tmp_path)
    manifest.json_save("{tmp_path}/{0}.json".format(index))

    return manifest


@dg.solid(config_schema=dg.StringSource)
def create_collection(context, manifest):
...
@dg.solid(config_schema=dg.StringSource)
def upload_to_cloud(context, collection):
    tmp_path = context.solid_config
...
#         collection = create_collection(manifest)
#         upload_to_cloud(collection)
